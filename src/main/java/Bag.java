...
public abstract class Bag {
    ...
    public boolean addItem (String item) {
        if(this.numberOfContents >= this.capacity){
            ...
            String[] newAddedItems = new String[this.contents.length + 1];

            for(int i = 0; i < this.contents.length; i++){
                newAddedItems[i] = this.contents[i];
            }

            newAddedItems[newAddedItems.length - 1] = item;
            this.contents = newAddedItems;
    ...
    public String popItem()  {
        if (this.numberOfContents == 0) {
            ...
            String[] newCopyOfBag = new String[this.contents.length - 1];

            for(int i = 0; i < newCopyOfBag.length; i++){
                newCopyOfBag[i] = this.contents[i];
            }

            this.contents = newCopyOfBag;
